...
try:
    ...
    def overlay_masks(image, masks, labels=None, colors=None, mask_alpha=0.5):
        image_arr = np.asarray(image)
        fig, ax = plt.subplots()

        if image_arr.ndim == 2:
            ax.imshow(image_arr, cmap='gray', interpolation='nearest')
        elif image_arr.ndim == 3 and image_arr.shape[-1] in (1, 3, 4):
            ax.imshow(image_arr, interpolation='nearest')
        else:
            ax.imshow(image_arr.squeeze(), cmap='gray', interpolation='nearest')

        mask_labels = labels if labels is not None else np.arange(1, len(masks) + 1)
        for idx, mask in enumerate(masks):
            mask_arr = np.asarray(mask).astype(float)
            label = mask_labels[idx] if idx < len(mask_labels) else idx + 1
            if isinstance(colors, dict):
                color = colors.get(label, colors.get(idx + 1, 'red'))
            elif isinstance(colors, (list, tuple)):
                color = colors[idx % len(colors)]
            else:
                color = 'red'

            overlay_rgba = mcolors.to_rgba(color, alpha=mask_alpha)
            cmap = ListedColormap([(0, 0, 0, 0), overlay_rgba])
            ax.imshow(mask_arr, cmap=cmap, interpolation='nearest', vmin=0, vmax=1)

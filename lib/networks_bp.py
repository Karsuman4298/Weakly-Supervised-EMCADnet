# networks_bp.py  —  EMCADNet for BPAnno (EAUWSeg)

#   cls_head  :  3-class CCG
#                  (certain BG / uncertain ring / certain FG)
#   forward() : returns preds, cls_logits, embeddings in train mode
...
class EMCADNet(nn.Module):
    ...
    def __init__(self,
        ...
        encoder_cfg = {
            ...
            'resnet18':  (lambda: resnet18(pretrained=pretrain),
                          None, [512, 256, 128,  64]),
            'resnet34':  (lambda: resnet34(pretrained=pretrain),
                          None, [512, 256, 128,  64]),
            'resnet50':  (lambda: resnet50(pretrained=pretrain),
                          None, [2048, 1024, 512, 256]),
            'resnet101': (lambda: resnet101(pretrained=pretrain),
                          None, [2048, 1024, 512, 256]),
            'resnet152': (lambda: resnet152(pretrained=pretrain),
                          None, [2048, 1024, 512, 256]),
        }
        ...
        # ------------------------------------------------------------------ #
        # 3-class CCG head  (BPAnno standard — EAUWSeg)
        #   0 = certain BG   (outside envelope polygon P_out)
        #   1 = uncertain    (inside P_out but outside P_in = ring)
        #   2 = certain FG   (inside inscribed polygon P_in)
        # ------------------------------------------------------------------ #
        self.cls_head = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(channels[3] // 2, 3, kernel_size=1)   # 3 classes
        )
    ...
    def forward(self, x, mode='test'):
        ...
        # BPAnno returns 3 things in train mode (no conf_map)
...
if __name__ == '__main__':
    ...
    print('cls_logits:     ', cls.shape)    # (2, 3, 352, 352)
